...
\item Create a repository named \texttt{stat-471-fall-2021} on your personal Github account by clicking ``Repositories'', clicking the green ``New'' button, choosing the options pictured in Figure~\ref{fig:new-github-repo}, and clicking the green ``Create repository'' button. 
\item Create a folder on your computer called \texttt{stat-471-fall-2021}.
...
@
\item Change the directory to \texttt{stat-471-fall-2021}. In RStudio, you can do this by navigating to the \texttt{stat-471-fall-2021} directory in the ``Files'' pane (Figure~\ref{fig:rstudio}E), clicking \texttt{More} -> \texttt{Set As Working Directory} (Figure~\ref{fig:rstudio}D), clicking the down arrow next to the ``Terminal" tab, and clicking \texttt{Go to Current Directory}. 
\item In the Terminal, type
<<eval=FALSE>>=
git clone https://github.com/katsevich-teaching/stat-471-fall-2021.git
@
This will copy the STAT 471 Github repository onto your local computer.
  \item In the Terminal, type
<<eval=FALSE>>=
