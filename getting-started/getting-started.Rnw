...
\item Click the \href{https://classroom.github.com/g/Kbt0vB3h}{course invitation link} (please do not share this link with students not in the class). Then, follow the prompts to click ``Authorize Github Classroom'', join ``STAT 471 Students (Fall 2021)'', and then navigate to the class Github repository. 
...
@
\item Change the directory to \texttt{stat-471-fall-2021}. In RStudio, you can do this by navigating to the \texttt{stat-471-fall-2021} directory in the ``Files'' pane (Figure~\ref{fig:rstudio}F), clicking \texttt{More} -> \texttt{Set As Working Directory} (Figure~\ref{fig:rstudio}E), clicking the down arrow next to the ``Terminal" tab, and clicking \texttt{Go to Current Directory}. 
