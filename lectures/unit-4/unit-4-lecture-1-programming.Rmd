...
```{r}
tree_fit_2 = rpart(Salary ~ ., control = rpart.control(minsplit = 80),
                   data = Hitters_train)
...
We can then extract the MSE of the two methods using `summarise`, as usual:
```{r}
results %>% summarise(MSE_1 = mean((Y - Y_hat_1)^2), 
                      MSE_2 = mean((Y-Y_hat_2)^2))
```
