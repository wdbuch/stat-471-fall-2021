---
title: 'Unit 4 Lecture 2: Pruning and cross-validating decision trees (Solutions)'
date: "April 6, 2021"
output:
...
# download the data
dataurl = "https://raw.githubusercontent.com/JWarmenhoven/ISLR-python/master/Notebooks/Data/Heart.csv"
Heart = read_csv(dataurl) %>% select(-X1)
...
**The `CP` column is the complexity parameter. In this case, it is the smallest improvement in the misclassification error that is acceptable before adding a split. They decrease as the size of the tree increases, which makes sense because there is less high of a bar for adding splits.**
...
**We would choose two terminal nodes. One strange thing in the CV plot is that there are "gaps" at 3 and 7 terminal nodes. This occurs because internal nodes, rather than leaf nodes, were the weakest link at those two places in the pruning algorithm, decreasing the number of terminal nodes by two rather than by one.**
...
**This decision tree classifies `No` if and only if a patient's chest pain falls into one of the following categories: nonanginal, nontypical, typical.**
