...
**Figure \@ref(fig:lasso-CV-plot) shows the lasso CV plot. The leftmost edge corresponds to $\lambda \approx 0$, i.e. the ordinary linear regression model. It performs poorly due to high variance.**
...
**Figure \@ref(fig:lasso-trace-plot) shows the lasso trace plot. The first feature entering the model with a positive coefficient is `segregation_nonwhite_white`. The first feature entering the model with a positive coefficient is `air_pollution`.**
